...
function CookieStandForm() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://cookiestandapi20231005154615.azurewebsites.net/api/CookieStands');
  ...
  const onCreate = async (newCookieStand) => {
    try {
      setLoading(true);
      const response = await axios.post('https://cookiestandapi20231005154615.azurewebsites.net/api/CookieStands', newCookieStand);
  ...
  const onDelete = async (id) => {
    try {
      ...
      // Send the delete request to the API
        const response = await axios.delete(`https://cookiestandapi20231005154615.azurewebsites.net/api/CookieStands/${id}`);
